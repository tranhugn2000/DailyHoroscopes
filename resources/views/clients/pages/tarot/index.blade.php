...
<div class="container mx-auto min-h-screen">
    <button class="inline-flex deal items-center justify-center px-5 py-2 text-base font-medium text-center text-indigo-100 border border-indigo-500 rounded-3xl shadow-sm cursor-pointer hover:text-white bg-gradient-to-br from-purple-500 via-indigo-500 to-indigo-500">
        <span class="relative">Reset <i class="fa-solid fa-arrow-rotate-left"></i></span>
    </button>
    <div class="deck my-6 text-center">
        @foreach ($cards as $key => $card)
        <div class="card card1 mt-5 hover:translate-y-[-0.75rem] z-[900] open-card" data-name="{{ $card['name'] }}" data-image="{{ asset($card['src']) }}" data-main-meaning="{{ $card['mainMeaning'] }}" data-reverse-meaning="{{ $card['reverseMeaning'] }}" data-description="{{ $card['description'] }}">
            ...
            <div class="face back  overflow-hidden ">
                <img class="object-fill w-full h-full" src="{{ asset($card['src']) }}" alt="">
            </div>
        </div>
        @endforeach
    </div>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.forEach(function(card, key) {
                    let cardElement = `
                        <div class="card card1 mt-5 hover:translate-y-[-0.75rem] z-[900] open-card" data-name="${card.name}" data-image="${card.src}" data-main-meaning="${card.mainMeaning}" data-reverse-meaning="${card.reverseMeaning}" data-description="${card.description}">
                            <div class="face front overflow-hidden border-[1.5px] border-gray-300">
                                <img class="object-contain w-full h-full" src="{{ asset('assets/images/cards/cart-tarot-3.jpg') }}" alt="">
                            </div>
                            <div class="face back overflow-hidden">
                                <img class="object-fill w-full h-full" src="${card.src}" alt="">
                            </div>
                        </div>
                    `;
                    $('.card').hide();
            ...
            let cardDetails = `
                            <div class="selected-card flex justify-center">
                                <div class="card-info h-full">
                                    <figure class="front"><img class="h-full w-full" src="${cardImage}" /></figure>
                                    <figure class="back">
    ...
    function readSelectedCards() {
        $.ajax({
            ...
            success: function(response) {
                ...
                console.log('Selected cards sent successfully:', response);
            },
            error: function() {
